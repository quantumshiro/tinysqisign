...
本資料では、公開成果物の完全性検査、公開ソースの再構成、新しい環境での再実験を区別する。新しい測定値は既存の測定記録と区別して保存する。
...
| v3二関数重畳版 | 同上 | `874658c64aa2e20f53b1f4d696144723d558ed5c` | `patches/v3-two-function-lifetime.bundle` |
| v3固定frame解析版 | 同上 | `cb94f242ba791a4ccb980b46c917830b309a9832` | `patches/v3-static-stack.bundle` |
...
固定frame treeだけを再生成する場合は`./scripts/generate_v3_pqm4.sh static`を使う。この解析版は`p324_3/RADIX32`専用であり、他のv3 parameterを対象にしない。
...
RP2350の測定時ビルド環境はPico SDK 2.3.0、Arm GNU Toolchain 15.2.Rel1、Ninja 1.13.2である。再ビルドには次を使う。
...
操作PSP上界は、測定ELFとbyte一致する `.text` を持つcommit固定の再構築版から次で再生成する。結果は入力依存Thread-mode call closureと最大Secure例外entry一回を含むが、handler/MSPは含まない。
...
二関数重畳版のsource contract、全parameterのhost response、Arm frame、および実機summaryは次で再検査できる。Arm解析の`--compiler`と`--size-tool`には使用するGNU Arm toolchainの実体を指定する。
...
- `interleaved-clean-2026-09-04/`: 未commit差分のないfirmwareによる公式版/適応版5組10 boot。適応ソースは基準commitとdigest固定patchで表現した。
- `multi-input-placement-clean-2026-09-04/`: commit固定の公式版と適応版、10公式vector、2配置。正当40試行と改変署名拒否40試行を含む。
- `static-closure-clean-2026-09-04/`: commit固定の固定frame source/firmware、公式vector 0、link済みoperation-PSP上界との照合。
- `fixed-key-timing-clean-2026-09-04/`: commit固定の公式/適応sourceとfirmware、10鍵、2順序、各5反復、合計200 Sign。
- `d3-two-function-clean-2026-09-04/`: commit固定の二関数重畳source/firmwareによる`p324_3`第0ベクトル。全三parameterのhost試験とArm frame監査は別JSONへ保存する。

測定firmware commitは順に`467ca5b61a5f6810218ee173850862d629cf07a7`、`76f88dccf6ce9ba1b4d12fc20724b47e03a5f6ab`、`e564a6413766f1f299db3db1d706478c42f1cc96`、`899e572f084c90b7b3be30222e340dc42e380876`、`5df7acfb2e32019305094546939bdc50bbf71e00`である。これらは測定時の履歴識別子であり、現在のartifact repository HEADではない。各summaryはELF/UF2/map/captureのdigestを保持する。
...
`experiments/sca/`には事前条件、入力設計、software trace、固定work実験、および残存する可変処理の記録がある。v3 target固定鍵screenは200/200署名を検証し、鍵に対応するwall-clock順位を二順序で再現した。公開鍵成分と秘密鍵成分が同時に変わるため、秘密だけへの因果帰属、物理漏洩、鍵回復、耐性を示さない。

電流/EM probeとscope/SCA取得器を用いた物理traceは本成果物に含まれない。software結果は物理漏洩の測定値として扱わない。
...
生成済み`manuscript/main.pdf`と`manuscript/main-en.pdf`も収録する。主要数値は`manuscript/generated/`へJSON/CSVから生成される。
