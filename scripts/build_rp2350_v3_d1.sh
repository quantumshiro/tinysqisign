#!/bin/sh
# Build the isolated SQIsign v3.0 p324_3 lifetime-scheduled D1 image.
set -eu

project_root=$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd -P)
sdk_root=${PICO_SDK_PATH:-"$project_root/external/pico-sdk"}
v3_root=${SQISIGN_V3_SOURCE_ROOT:-"$project_root/work/v3-lowmem-d1"}
build_root=${SQISIGN_RP2350_V3_BUILD_ROOT:-"$project_root/build-rp2350-v3-d1"}
...
expected_v3_commit=9293313fb58de4c5ce9dd27a5a9fde0058766c79
...
PATH="$toolchain_root/bin:$PATH" \
cmake --build "$build_root" --target sqisign_rp2350_v3_d1 --parallel

elf="$build_root/sqisign_rp2350_v3_d1.elf"
test -f "$elf"
...
printf 'RP2350 SQIsign v3 D1 build PASS\nelf=%s\nuf2=%s\nmap=%s.map\n' \
    "$elf" "${elf%.elf}.uf2" "$elf"
