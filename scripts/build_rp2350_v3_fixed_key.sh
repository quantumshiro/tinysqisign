#!/bin/sh
# Build an official-v3 or D1 fixed-message/fixed-RNG multi-key timing image.
set -eu
...
case "$kind" in
    baseline|d1) ;;
    *) printf 'kind must be baseline or d1\n' >&2; exit 2 ;;
esac
...
cmake \
    ...
    -DSQISIGN_V3_IMAGE_KIND="${kind}-sca-key" \
...
target="sqisign_rp2350_v3_${kind}_sca_key"
PATH="$toolchain_root/bin:$PATH" \
...
printf 'RP2350 SQIsign v3 fixed-key build PASS\nkind=%s\nelf=%s\nuf2=%s\ngenerated_tree_sha256=%s\n' \
    "$kind" "$elf" "${elf%.elf}.uf2" "$generated_digest"
