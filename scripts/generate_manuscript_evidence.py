...
def main() -> int:
    ...
    # Translate every generated table row, retaining all cells and numeric tokens.
    # Unknown Japanese text is an error, rather than silently dropping a row.
    translations = {
        "オンチップSRAM総量": "On-chip SRAM total",
        "主バンク＋補助バンク": "Main and auxiliary banks",
        "操作用アリーナ本体": "Operation arena body",
        "KeyGen・Sign・Verifyの共有領域": "KeyGen/Sign/Verify shared region",
        "保護領域込み操作用共有領域": "Guarded operation region",
        "主バンク": "Main bank",
        "予約": " reservation",
        "その他の主バンク領域": "Other main-bank regions",
        "データ・実行時状態等": "Data, runtime state, etc.",
        "主バンクのリンク時使用量": "Main-bank linked use",
        "上記の合計": "Total of the above",
        "補助バンク": "Auxiliary bank",
        "SRAM予約合計": "Total SRAM reservation",
        "全精度表比": "Relative to full table: ",
        "未予約SRAM": "Unreserved SRAM",
        "ヒープ": "Heap",
        "動的メモリ確保シンボルなし": "No dynamic-allocation symbols",
        "XIPフラッシュ": "XIP flash",
        "フラッシュからRAMへ読込み": "Loaded from flash into RAM",
        "RAM、予約を含む": "RAM, including reservations",
        "バイナリイメージ": "Binary image",
        "最大固定スタックフレーム": "Largest fixed stack frame",
        "PSP使用量の実測値": "PSP watermark",
        "暗号ライブラリの命令領域": "Cryptographic-library instructions",
        "時間の中央値": " median time",
        "時間": " time",
        "正当なKeyGen・Sign・Verify": "Valid KeyGen/Sign/Verify",
        "改変署名の拒否": "Modified-signature rejection",
        "配置A/B間PSP不一致": "PSP mismatches across layouts A/B",
        "対応時間差の範囲": "Paired timing-change range",
        "動的局所フレーム記録": "Dynamic local-frame records",
        "ホスト上の公式応答": "Host official responses",
        "フレーム": " frame"
    }
    for basename in (
        "v2-memory", "v3-results", "v3-d3-results",
        "v3-multi-results", "v3-d2-static",
    ):
        source = args.output_dir / f"{basename}-rows.tex"
        translated = source.read_text(encoding="utf-8")
        # Historical measurement tables retain their original labels. Normalize
        # only publication copies; do not rewrite captured evidence.
        translated = translated.replace("PSP上書き深さ", "PSP使用量の実測値")
        source.write_text(translated, encoding="utf-8")
        for ja, en in sorted(translations.items(), key=lambda item: -len(item[0])):
            translated = translated.replace(ja, en)
        require(not any("\u3000" <= c <= "\u9fff" for c in translated),
                f"untranslated table text in {source}")
        (args.output_dir / f"{basename}-rows-en.tex").write_text(
    ...
    audit = {
        ...
        "outputs": [
            ...
            "manuscript/generated/v3-results-rows-en.tex",
            "manuscript/generated/v3-multi-results-rows-en.tex",
            "manuscript/generated/v3-d2-static-rows-en.tex",
